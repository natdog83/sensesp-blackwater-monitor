...
The design is based on the approach demonstrated by [Techstyleuk's SensESP black-water sensor array](https://github.com/Techstyleuk/SensESP-blackwater-tank-sensor-array), with updated Signal K paths, safer ESP32 pins, an explicit 12 V-to-3.3 V interface, sensor-state diagnostics, invalid-pattern detection, host tests, a serial hardware-test image, and automated builds.
...
The XKC-Y25-V yellow output can rise toward the sensor supply voltage. **Never connect it directly to an ESP32 GPIO.** Use the documented transistor interface (or a correctly engineered equivalent) and a fused, regulated 5 V supply for the ESP32.
